...
class CustomLoss(nn.Module):
    ...
    def forward(self, outputs, targets, skip_connections):
        ...
        skip_output1 = skip_connections[0]
        skip_output2 = skip_connections[1]
        skip_loss_component1 = torch.mean(skip_output1)
        skip_loss_component2 = torch.mean(skip_output2)
        # Additional loss component: L2 regularization on the final output
        l2_loss_component = torch.mean(outputs**2)
        total_loss = ce_loss + 0.005 * skip_loss_component1 + 0.005 * skip_loss_component2 + 0.001 * l2_loss_component  # Weighting the components
        return total_loss
