...
parser.add_argument('--plant', type=str, required=True, help='Plant species to classify: tomato, grape, apple, corn, potato')
...
plant = args.plant.lower()

# Get the directories for the specified plant
if plant not in plant_categories:
    raise ValueError(f"Unknown plant species: {args.plant}")

plant_classes = plant_categories[plant]['healthy'] + plant_categories[plant]['diseased']

# Create a mapping for healthy vs. not healthy
class_mapping = {plant_categories[plant]['healthy'][0]: 0}
for cls in plant_categories[plant]['diseased']:
    class_mapping[cls] = 1
...
# val_dir = f"/cs/home/psxjs24/data/PlantVillage/color"
...
full_dataset = PlantVillageDataset(train_dir, transform=data_transforms['train'])
...
best_model_path = f"best_model_{args.model}_{args.plant}.pth"
...
roc_file = f"roc/roc_data_{args.model}_{args.plant}_{unique_id}.csv"
...
# Save the true labels and predicted probabilities to a separate CSV file
